...
--Runtime: 5119 ms, faster than 36.92% of MS SQL Server online submissions for Sales Analysis III.
--Memory Usage: 0B, less than 100.00% of MS SQL Server online submissions for Sales Analysis III.

SELECT P.product_id,P.product_name
FROM (
	--- 1. every product with their sale date range
    SELECT
        S.product_id,
        MIN(S.sale_date) AS 'start_date',
        MAX(S.sale_date) AS 'end_date'
    FROM Sales S
    GROUP BY S.product_id
) AS SD
--- 3. join Product table to get product name
LEFT JOIN Product P ON SD.product_id = P.product_id
--- 2. filter product in 1st quarter in 2019
WHERE SD.start_date >= '2019-01-01' AND SD.end_date <= '2019-03-31';